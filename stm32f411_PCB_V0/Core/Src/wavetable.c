...
static BellowsMode previous_bellows_mode = MODE_PUSH;
...
static float Bellows_Gain(void)
{
    ...
    {
        float span = 4095.0f - (float)pressure_rest;
        gain = (span > 1.0f) ? ((float)pressure - (float)pressure_rest) / span : 0.0f;
    }
    else
    {
        float span = (float)pressure_rest;
        gain = (span > 1.0f) ? ((float)pressure_rest - (float)pressure) / span : 0.0f;
    }
...
static void Dispatch_Buttons(void)
{
    ...
    /* Changement de sens du soufflet pendant qu'un bouton reste enfonce :
       les boutons deja tenus doivent basculer vers la note de l'autre sens. */
    uint8_t bellows_changed = (bellows_mode != previous_bellows_mode);
    ...
    for(int i = 0; i < NB_BUTTONS; i++)
    {
        ...
        */
        ...
        else if(state && previous_buttons[i] && bellows_changed && active_sound[i] != NULL)
        {
    ...
    previous_bellows_mode = bellows_mode;
}
