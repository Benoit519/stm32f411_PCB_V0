...
typedef struct
{
    ...
    */
    ...
    /* wave = table de MAINTIEN (band-limited) ; wave_attack = table de
       DEBUT DE NOTE (plein spectre), jouee tant que env_state==ENV_ATTACK. */
    const int16_t *wave;
    const int16_t *wave_attack;
...
static void Voice_SetWave(Voice *v, WaveTableId wt)
{
    ...
    case WT_ACCORDION:
    {
        ...
        v->wave_attack = wavetable_accordion_attack[wi];
        break;
    }
    default:
        v->wave        = wavetable_accordion[0][0];
        v->wave_attack = wavetable_accordion_attack[0];
        break;
    }
}
